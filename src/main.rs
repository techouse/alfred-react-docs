...
use alfred_react_docs::app::{google_fallback_item, items_from_results, placeholder_item};
use alfred_react_docs::models::SearchResult;
use alfred_react_docs::services::AlgoliaSearch;
use alfred_workflow_rs::{Icon, Item, RenderOptions, Updater, UserConfiguration, Workflow};
use anyhow::Result;

use crate::cli::Cli;

const GITHUB_REPOSITORY_URL: &str = "https://github.com/techouse/alfred-react-docs";
...
fn populate_workflow(workflow: &mut Workflow, cli: &Cli) -> Result<()> {
    ...
    populate_workflow_with(workflow, cli, &settings, |query| {
...
fn read_workflow_settings(
    ...
    Ok(workflow_settings_from_defaults(&defaults))
}
