...
/// Configuration required to query the React Algolia index.
...
/// Synchronous client for the React Algolia index.
#[derive(Clone)]
...
impl AlgoliaSearch {
    ...
    /// Searches React documentation and returns at most 20 ranked hits.
    pub fn query(&self, query: &str) -> Result<Vec<SearchResult>> {
        self.query_with(query, |endpoint, body, remaining| {
    ...
    fn query_with<F>(&self, query: &str, mut attempt: F) -> Result<Vec<SearchResult>>
    where
    ...
    fn request_body(&self, query: &str) -> Result<String> {
