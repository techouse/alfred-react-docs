...
fn request_body_preserves_react_search_contract() -> Result<()> {
    let client = AlgoliaSearch::with_base_url(config(), Url::parse("http://localhost/")?)?;
    let body: Value = serde_json::from_str(&client.request_body("state hooks")?)?;
...
fn query_retries_retryable_failures_in_host_order() -> Result<()> {
    ...
    let hits = client.query_with("state hooks", |endpoint, _, _| {
    ...
    assert_eq!(
        endpoints,
        vec![
            "http://first.test/1/indexes/react/query",
            "http://second.test/1/indexes/react/query",
            "http://third.test/1/indexes/react/query",
        ]
    );
    Ok(())
}
...
fn query_reports_all_retryable_host_failures() -> Result<()> {
    ...
    let error = client
        .query_with("state hooks", |endpoint, _, _| {
            attempts += 1;
            Err(AttemptFailure::Retryable(anyhow::anyhow!(
                "{endpoint} failed"
            )))
    ...
    assert!(message.contains("first.test/1/indexes/react/query failed"));
    assert!(message.contains("second.test/1/indexes/react/query failed"));
    Ok(())
}
...
fn query_uses_one_shared_timeout_budget() -> Result<()> {
    ...
    let _ = client.query_with("state hooks", |_, _, remaining| {
...
fn query_falls_back_after_5xx_response() -> Result<()> {
    ...
    let hits = client.query("state hooks")?;
...
#[test]
fn query_rejects_malformed_successful_response() -> Result<()> {
    let (url, server) = serve_once(200, "not json")?;
    let client = AlgoliaSearch::with_read_hosts_and_agent(config(), vec![url], no_proxy_agent())?;

    let error = client
        .query("state hooks")
        .expect_err("malformed successful responses must fail");
    server
        .join()
        .map_err(|_| anyhow::anyhow!("server thread panicked"))?;
    ...
    assert!(
        error
            .to_string()
            .contains("failed to deserialize Algolia response")
    );
    Ok(())
}
